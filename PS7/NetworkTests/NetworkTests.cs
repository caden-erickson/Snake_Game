...
namespace NetworkUtil
{
    ...
    public class NetworkTests
    {
        ...
        public void TestStopServer()
        {
            ...
            // the server is stopped, so calling AcceptSocket should throw an exception
            Assert.ThrowsException<InvalidOperationException>(() => testListener.AcceptSocket());
        }

        [DataTestMethod]
        [DataRow("2001:0db8:85a3:0000:0000:8a2e:0370:7334")]
        [DataRow("googly.commm")]
        [DataRow("ipv6.google.com")]
        public void TestConnectInvalidAddress(string hostName)
        {
            void throwaway(SocketState? s)
            {
                return;
            }

            Networking.ConnectToServer(throwaway, hostName, 1221);

            // If it gets to this point it passes
        }

        //[DataTestMethod]
        //public void TestCloseMidGetData()
        //{
        //    void throwaway(SocketState? s)
        //    {
        //        return;
