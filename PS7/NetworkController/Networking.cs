...
// Changelog:
//    11/9/22 -- finished implementation
...
public static class Networking
{
    ...
    {
        // Create new TCP listener, start it
        TcpListener listener = new(IPAddress.Any, port);
        listener.Start();

        // Package together the connection delegate and the listener, pass to BeginAcceptSocket
        Tuple<TcpListener, Action<SocketState>> package = new(listener, toCall);
    ...
    {
        Tuple<TcpListener, Action<SocketState>> package = (ar.AsyncState as Tuple<TcpListener, Action<SocketState>>)!;

        try
        {
            // BETTER NAMES
            Socket socket = package.Item1.EndAcceptSocket(ar);
            SocketState state = new(package.Item2, socket);
            state.OnNetworkAction(state);
            package.Item1.BeginAcceptSocket(AcceptNewClient, package);
        }
        catch (Exception ex)
        {
            //Indicate an error to the user, as specified in the documentation
            //Ends event loop
            package.Item2(new SocketState(package.Item2, ex.Message));
        }
    }
    ...
    public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
    {
        ...
        catch (Exception)
        {
            ...
            {
                //Indicate an error to the user, as specified in the documentation
                toCall(new SocketState(toCall, x.Message));
            }
        }
        ...
        //Create a new socket state with the socket we just created
        SocketState state = new(toCall, socket);

        // Package together the connection delegate and the listener, pass to BeginAcceptSocket
        Tuple<Socket, Action<SocketState>> package = new(socket, toCall);

        //Watch for connection errors when calling BeginConnect
        try
        {
            //Begin connecting
            IAsyncResult ar = socket.BeginConnect(ipAddress, port, ConnectedCallback, package);

            //Timeout check, produce an error if we don't connect within 3 seconds.
            bool connected = ar.AsyncWaitHandle.WaitOne(3000);
    ...
    {
        //Pull our Tuple that contains the Socket and the toCall method
        Tuple<Socket, Action<SocketState>> package = (ar.AsyncState as Tuple<Socket, Action<SocketState>>)!;

        try
        {
            //End the connection process with our socket.
            package.Item1.EndConnect(ar);

            //Call the toCall method
            package.Item2(new SocketState(package.Item2, package.Item1));
        }
        catch (Exception ex)
        {
            //An error occured, take care of it as specified.
            package.Item2(new SocketState(package.Item2, ex.Message));
        }
    }
    ...
    public static void GetData(SocketState state)
    {
        ...
        {
            //Update the SocketState to show that an error occurred
    ...
    private static void ReceiveCallback(IAsyncResult ar)
    {
        ...
        try
        {
            ...
            if (numBytes == 0)
                throw new Exception("The socket was closed while receiving.");

            string msg = Encoding.UTF8.GetString(state.buffer, 0, numBytes);

            //Thread-Safe
            lock (state.data)
            {
                state.data.Append(msg); 
            }
    ...
    public static bool Send(Socket socket, string data)
    {
        try
        {
            // Per the documentation, Connected is not thread-safe
            lock (socket)
            {
                if (!socket.Connected)
                    throw new Exception();
